...
def _bpe_tokenize(self, token: str) -> List[str]:
    ...
    for token in text.split():
        ...
        if len(subwords) > 500:
            # If we've exceeded max iterations, try to find meaningful subwords
            return self._fallback_tokenize(token)

        # Try to find optimal length subwords first
        found = False
        # Start with ideal length and expand if needed
        for target_len in range(
            ideal_len, max_len + 1, 2
            ...
            if not found:
                # Try to find meaningful subwords
                subword = self._find_meaningful_subword(token[start:])
                if subword:
                    subwords.extend(subword)
                    start += len("".join(subword))
                else:
                    # Fallback to character-level tokenization
                    subwords.extend(self._char_tokenize(token[start:]))
                    break
        ...
        for subword in subwords:
            if len(current) > 0 and len(current[-1]) + len(subword) <= ideal_len:
                current.append(subword)
            else:
                if len(current) > 0:
                    optimized_subwords.append("".join(current))
                current = [subword]

        if len(current) > 0:
            optimized_subwords.append("".join(current))

        return optimized_subwords
